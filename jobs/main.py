...
def generate_weather_data(device_id, timestamp, location):
    return {
        ...
        'location': location,
        'timestamp': timestamp,
        'temperatue': random.uniform(-5, 26),
...
def generate_emergency_incident_data(device_id, timestamp, location):
    return{
        ...
        'timestamp': timestamp,
        'location': location,
        'status': random.choice(['Activate', 'Resolved']),
        'description': 'Descripton of the incident'
    }
...
def generate_vehicle_data(devide_id):
...
def simulate_journey(producer, device_id):
    while True:
        ...
        if (vehicle_data['location'][0] >= BIRMINGHAM_COORDINATES['latitude']
            ...
            print('Vehicle has reached Birminghan. Simulation ending...')
            break 
